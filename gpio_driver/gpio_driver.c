...
static irqreturn_t h_irq_gpio22(int irq, void *data)
{
	...
	reset_counter(); // LEDs will start from 0000
	reset_buffer();  // user will see 00:00:00
    }
...
char pins[4] = {GPIO_06, GPIO_13, GPIO_19, GPIO_26};

void set_diodes(int n)
{
    clear_all(); // turn off all diodes
    int combination = convert_to_binary(n);  // get the binary number (we know it will be 0-15, so 4 bites)
    int digit;
    int i = 0;
...
void update_buffer(void)
{
    ...
    char ss[3];
    sprintf(ss, "%d", stopwatch.ss);
    char mm[3];
    sprintf(mm, "%d", stopwatch.mm);
    char hh[3];
    sprintf(hh, "%d", stopwatch.hh);

    // append everything on gpio_driver_buffer
    if(stopwatch.hh < 10)
    {
	strcpy(gpio_driver_buffer, "0");
	strcat(gpio_driver_buffer, hh);
    }
    else
	strcpy(gpio_driver_buffer, hh);
    strcat(gpio_driver_buffer, ":");

    if(stopwatch.mm < 10)
	strcat(gpio_driver_buffer, "0");
    strcat(gpio_driver_buffer, mm);

    strcat(gpio_driver_buffer, ":");

    if(stopwatch.ss < 10)
	strcat(gpio_driver_buffer, "0");
    strcat(gpio_driver_buffer, ss);
}

void reset_buffer(void)
{
    char* reset_state = "00:00:00";
    strcpy(gpio_driver_buffer, reset_state);
}

void reset_stopwatch(void)
{
    stopwatch.hh = stopwatch.mm = stopwatch.ss = 0;
}
